...
import { getPostBySlug, getAllPostsIncludingDrafts, PostFrontmatter } from "@/lib/blog";
...
const PostEditor = () => {
  ...
  // Editable state initialized from real frontmatter
  const [fm, setFm] = useState<PostFrontmatter>({ ...original });
  const [copied, setCopied] = useState(false);

  const hasChanges = useMemo(() => {
    return JSON.stringify(fm) !== JSON.stringify(original);
  }, [fm, original]);
...
---`;
  ...
  function copyForClaude() {
    const prompt = `Please update the frontmatter in src/content/posts/${fm.slug}.mdx with these changes and push to GitHub:\n\n${generateFrontmatter()}`;
            ...
            {hasChanges && (
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 space-y-3">
                ...
                <p className="text-xs text-amber-700">
                  Copy the updated frontmatter below and paste it to Claude to push the changes to GitHub.
                </p>
                <button
                  onClick={copyForClaude}
